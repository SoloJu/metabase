...
describe("Google Drive > DB Menu", () => {
  ...
  it("reassures (transient) when erroring but synced within the last hour", async () => {
    // Synced before + erroring now => most likely a transient failure it's
    // auto-recovering from. Reassure instead of the scary error (CLO-5546).
    setup({
      status: "error",
  ...
  it("warns of delay when erroring and last synced over an hour ago", async () => {
    setup({
      status: "error",
